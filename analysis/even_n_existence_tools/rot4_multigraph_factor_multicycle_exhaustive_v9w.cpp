...
static bool factor_dfs(std::array<int, 37>& deficit) {
    ++current_nodes;
    U64 state = encode_state(deficit);
    if (state == 0) {
        witness_cells = current_cells;
        return true;
    }
    if (line_remaining.empty() && failed_states.count(state)) return false;
    ...
    if (need == 1) {
        for (const Cell& cell : usable_nonloops(chosen, deficit)) {
            ...
            if (factor_dfs(deficit)) return true;
            adjust_cell(cell.id, 1);
...
int main() {
    ...
    // v9w: for explicit-mask mode, record the completed factor (current_cells)
    // for EVERY feasible mask so survivors can be geometrically verified.
    ...
    auto test_mask = [&](U64 removed) {
        ...
        bool feasible = factor_dfs(deficit);
        ...
        if (feasible) {
            ...
            if (enumeration_mode == 3)
                witness_factors.emplace_back(removed, current_cells);
        }
