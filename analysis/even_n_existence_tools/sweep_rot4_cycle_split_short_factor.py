"""Split one fixed window profile by deletions in a uniquely sized cycle."""
...
def main() -> None:
    ...
    parser.add_argument("--cycle-deletion-counts", required=True)
    ...
    counts = [
        int(value) for value in args.cycle_deletion_counts.split(",")
    ]
    summary = {
        "parameters": vars(args),
        "expected_cycle_deletion_counts": counts,
        "layers": [],
    }
    ...
    for count in counts:
        tag = (
            ...
            f"_cycle{args.cycle_length}d{count}_norm_q{args.direction_q}hard"
        )
