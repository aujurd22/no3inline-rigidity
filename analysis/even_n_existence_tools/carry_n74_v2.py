"""
修正版 N=74 进位+SA：碰撞映射使用整数进位值。
关键修复：collision map 用 π_int = π_mod + carry[x]*N (真整数行列式)。
SA 交换 π_mod[i]↔π_mod[j]，进位 stay at position。
"""
...
def build_int_collision_map(pi_mod, carry, N_val):
    ...
    for i, j, k in itertools.combinations(range(N_val), 3):
        yi0 = pi_mod[i] + carry[i]*N_val
        yj0 = pi_mod[j] + carry[j]*N_val
        yk0 = pi_mod[k] + carry[k]*N_val
        # PPP
        if (j-i)*(yk0-yi0) == (k-i)*(yj0-yi0):
            collisions[(i,j,k,'PPP')] = True
        # PPS
        yk_s = N_val-1 - (pi_mod[k] + carry[k]*N_val)
        if (j-i)*(yk_s-yi0) == (k-i)*(yj0-yi0):
            collisions[(i,j,k,'PPS')] = True
        # PSS
        yj_s = N_val-1 - (pi_mod[j] + carry[j]*N_val)
        if (j-i)*(yk_s-yi0) == (k-i)*(yj_s-yi0):
...
def sa_absorb_int(pi_mod, carry, N_val, max_iter=20000, patience=400):
...
for trial in range(3):
    ...
    # 2. 进位提升（50次尝试，检查全部3种冲突）
    best_cnt = float('inf')
    best_pi, best_carry = None, None
    n_attempts = 50
    
    for ca in range(n_attempts):
        ...
        if z == 0:
            # 完整 NTIL 验证
            pts = [(x, pi[x] + carry[x]*N_val) for x in range(N_val)] + \
                  [(x, N_val-1 - (pi[x] + carry[x]*N_val)) for x in range(N_val)]
            if not any((x2-x1)*(y3-y1) == (x3-x1)*(y2-y1)
                       for (x1,y1),(x2,y2),(x3,y3) in itertools.combinations(pts, 3)):
                best_cnt = 0; break
    
    carry_time = time.time() - t0
    print(f"  carry: {n_attempts} attempts, best residual = {best_cnt} ({carry_time:.1f}s)")
    
    if best_cnt == 0:
        print(f"  ★ carry直接解!")
        json.dump({'N': N_val, 'pi': best_pi, 'carry': best_carry, 'method': 'carry'},
                  open('ntil_solution_n74.json', 'w'), indent=2)
        continue
    
    # 3. SA 吸收（整数版）
    print(f"  SA starting with {best_cnt} residual conflicts...")
    t1 = time.time()
    final, swaps, rsts = sa_absorb_int(best_pi, best_carry, N_val, max_iter=20000, patience=400)
    elapsed = time.time() - t1
    
    if final == 0:
        print(f"  ★ SA吸收成功! swaps={swaps} rsts={rsts} ({elapsed:.1f}s)")
        json.dump({'N': N_val, 'pi': best_pi, 'carry': best_carry, 'method': 'carry+SA',
