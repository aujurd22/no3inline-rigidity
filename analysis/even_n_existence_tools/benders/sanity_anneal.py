"""通用退火健全性检查：给定 m (N=2m 网格 rot4 NTIL) 应存在解（若文献确证有解）。
...
用法：python sanity_anneal.py <m> [max_steps]
"""
import sys, random, json, itertools, math, time
...
def two_edge_switch(edges, rng):
    ...
    new = sorted(edges[:i] + ne + edges[j+1:])
    deg = {}
    for u, v in new:
        deg[u] = deg.get(u, 0) + 1
        deg[v] = deg.get(v, 0) + 1
...
def main():
    ...
    json.dump(out, open(f"sanity_anneal_m{M}_result.json", "w"), ensure_ascii=False, indent=2)
    print("DONE m=%d found=%s final_E=%d step=%s" % (M, found is not None, E, (found["step"] if found else None)), flush=True)
