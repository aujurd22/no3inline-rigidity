"""隔离健全性检查：m=36 (N=72 网格 rot4 NTIL) 应存在解。
目的：验证本工作模型在 m<=36 能找到 3-CNF SAT（进而几何可行解），
消除"m=37 全 UNSAT 是模型假阴性"的风险。完全隔离，不加载 m=37 环境。
结论若找到 sat_solution => 模型健全，m=37 的 UNSAT 是真实的（在采样空间内）。
"""
import random, json, itertools, math, time, sys

# ---- 本地参数：m=36 => N=72 网格，36 cell ----
N = 72
M = 36
...
def main():
    ...
    for t in range(TRIALS):
        ...
        status, model = dpll_model(clauses, nvars, node_budget=20_000_000)
        if status is True:
            v = "unsat"
        elif status is None:
            v = "unknown"
    ...
    json.dump(out, open("sanity_m36_result.json", "w"), ensure_ascii=False, indent=2)
